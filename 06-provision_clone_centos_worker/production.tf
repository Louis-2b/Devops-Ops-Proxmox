...
resource "proxmox_virtual_environment_vm" "worker" {
  # -- Paramètres généraux
  vm_id     = 355                              # ID unique de la VM
  name      = "node-1"                         # Nom de la VM
  node_name = "pve1"                           # Nom du nœud Proxmox
  tags      = ["terraform", "worker", "clone"] # Tags pour organiser les VMs
  ...
  initialization {
    ip_config {
      ipv4 {
        address = "192.168.222.95/24" # IPs statiques pour les VMs
...
output "vm_ip" {
  value       = proxmox_virtual_environment_vm.worker.initialization[0].ip_config[0].ipv4[0].address
  description = "Adresse IP de la worker"
}