...
resource "proxmox_virtual_environment_vm" "Bind9" {
  # -- Paramètres généraux
  vm_id     = 351                             # ID unique de la VM
  name      = "Bind9"                         # Nom de la VM
  node_name = "pve1"                          # Nom du nœud Proxmox
  tags      = ["terraform", "Bind9", "clone"] # Tags pour organiser les VMs
  ...
  initialization {
    ip_config {
      ipv4 {
        address = "192.168.222.91/24" # IPs statiques pour les VMs
...
output "vm_ip" {
  value       = proxmox_virtual_environment_vm.Bind9.initialization[0].ip_config[0].ipv4[0].address
  description = "Adresse IP de la Bind9"
}