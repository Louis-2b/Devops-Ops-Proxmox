...
resource "proxmox_virtual_environment_vm" "centos_template" {
  ...
  name        = "centos9-template"                   # Nom de la VM
  node_name   = var.node_name                        # Nom du nœud Proxmox sur lequel la VM sera créée
  tags        = ["terraform", "centos9", "template"] # Tags pour organiser les ressources
  ...
  memory {
    dedicated = 2048 # Mémoire dédiée en Mo
    floating  = 2048 # Mémoire flottante (gonflement), définir égal à la mémoire dédiée pour activer le gonflement
  }
  ...
  disk {
    ...
    file_id      = proxmox_virtual_environment_download_file.latest_CentOS-Stream-GenericCloud-9_qcow2_img.id
  ...
  initialization {
    ip_config {
      ipv4 {
        address = "192.168.222.90/24" # Adresse IP statique
...
resource "proxmox_virtual_environment_download_file" "latest_CentOS-Stream-GenericCloud-9_qcow2_img" {
  content_type = "iso"
  datastore_id = var.datastore_id
  file_name    = "CentOS-Stream-GenericCloud-9-latest.x86_64.qcow2.img"
...
output "vm_ip" {
  value       = proxmox_virtual_environment_vm.centos_template.initialization[0].ip_config[0].ipv4[0].address
  description = "Adresse IP de la VM centos_vm"
}