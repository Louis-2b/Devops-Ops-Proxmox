...
resource "proxmox_virtual_environment_vm" "HAproxy" {
  # -- Paramètres généraux
  count     = 2
  vm_id     = 353 + count.index  # ID unique pour chaque VM (352, 353)
  name = "HAproxy${count.index + 1}"             # Nom de la VM
  node_name = "pve1"                # Nom du nœud Proxmox
  tags      = ["terraform", "HAproxy", "clone"] # Tags pour organiser les VMs
  ...
  initialization {
    ip_config {
      ipv4 {
        address = "192.168.222.9${count.index + 3}/24" # IPs : 192.168.222.93, 192.168.222.94
...
output "vm_ips" {
  value       = [for vm in proxmox_virtual_environment_vm.HAproxy : vm.initialization[0].ip_config[0].ipv4[0].address]
